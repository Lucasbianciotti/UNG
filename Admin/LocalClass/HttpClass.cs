...
namespace Admin.LocalClass
{
    public static class HttpClass
    {
        ...
        public static async Task<Dashboard_Request> Get_Dashboard(HttpClient Http, FilterDashboard_Request filtros, IToast_Services _Toast)
        {
            try
            {
                HttpResponseMessage post = await Post(Http, URLsAdmin.Dashboard, filtros, _Toast);
        ...
        public static void NuevoLog(HttpClient Http, string comentario, SystemActionsEnum accion, SystemTypesEnum sistema, Exception exception, SystemErrorCodesEnum codigo)
        {
            if (exception != null)
            {
                try
                {
                    ...
                    Task.Run(async () =>
                    {
                        var post = await Http.PostAsJsonAsync(URLsAdmin.Errores, model);
                    });
                }
                catch (Exception)
                {
        ...
        private static async Task<HttpResponseMessage> Post(HttpClient Http, string URL, object model, IToast_Services _Toast)
        {
            ...
            {
                HttpClass.NuevoLog(Http, "No se pudo conectar con el servidor.", SystemActionsEnum.POST, SystemTypesEnum.WEB, e, SystemErrorCodesEnum.Error);
        ...
        private static async Task<HttpResponseMessage> Get(HttpClient Http, string URL, IToast_Services _Toast)
        {
            ...
            {
                HttpClass.NuevoLog(Http, "No se pudo conectar con el servidor.", SystemActionsEnum.GET, SystemTypesEnum.WEB, e, SystemErrorCodesEnum.Error);
                _Toast.ShowError("No se pudo conectar con el servidor. Verifique su conexión a internet.");
                return null;
            }
        }
